...
import bcrypt #it will help to generate and compare hash value sor our password
...
def authenticate():
    ...
    try:
        # Retrieve the user details for the given username
        cursor.execute("SELECT * FROM users WHERE username = %s", (username,))
        user = cursor.fetchone()  # Fetch the matching row

        if user:
            stored_hashed_password = user[2]  # Assuming the hashed password is in the 3rd column
            ...
            if bcrypt.checkpw(password.encode(), stored_hashed_password.encode()):
                ...
                if entered_otp == otp:
                    ...
                    return {"username": user[1], "role": user[3]}  # Assuming user[1] is the username column
                else:
                    print("Invalid OTP!")
                    return None
            else:
